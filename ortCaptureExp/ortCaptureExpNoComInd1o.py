...
def ann_inputs_outputs(preds, preds_n, prey, net):
...
def ann_inputs_outputs_t(tpreds, tpred_n, tprey, net):
    tpreds_copy = copy.deepcopy(tpreds)
    tpreds_ordered = []
    tpreds_ordered.append(tpreds_copy[tpred_n])
...
def simula1(net, preds, prey, height, width, ticks):
    ...
    for count in range(ticks):
        ...
        for npred in range(n_preds):#for pred, signal in zip (preds, signals):
            tpred = tpreds[npred]
            output= ann_inputs_outputs_t(tpreds, npred, tprey, net)[0]
...
def eval_fitness1(net, preds_def, theprey, height, width, ticks):
    ...
    for count in range(ticks): #(500 * 2 / 10) * (3/2) = 150
        ...
        for npred in range(n_preds):#for pred, signal in zip (preds, signals):
            pred = preds[npred]
            output= ann_inputs_outputs(preds, npred, prey, net)[0]
