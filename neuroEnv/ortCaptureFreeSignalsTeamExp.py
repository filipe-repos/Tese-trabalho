...
def ann_inputs_outputs_signal(preds, signals, prey, net):
...
def ann_inputs_outputs_signal_t(tpreds, signals, tprey, net):
...
def simula1(net, preds, prey, height, width, ticks):
    ...
    for count in range(ticks):
        ...
        outputsbruto, newsignals = ann_inputs_outputs_signal_t(tpreds, signals, tprey, net)
        signals = newsignals
        outputs = []
        n=0
        ...
        for pred, output in zip(tpreds, outputsbruto):
            tpred_move(pred, output, STEP)
...
def eval_fitness1(net, preds_def, theprey, height, width, ticks):
    ...
    for count in range(ticks): #(500 * 2 / 10) * (3/2) = 150
        ...
        outputsbruto, newsignals = ann_inputs_outputs_signal(preds, signals, prey, net)
        signals = newsignals
        outputs = []
        n=0
        #for n in range(0, (n_preds-1)*5 +1, 5):
        #    outpred = outputsbruto[n:5+n]
        #    outputs.append(outpred)
        for pred, output in zip(preds_def, outputsbruto):
    ...
    if all(i for i in onedirectionpreds):
        return 0 #bad fitness if all preds only moved in one direction during evolution
    if equalsx or equalsy:
        return 0 #bad fitness if not capture and pos of pred in x or y are equal
