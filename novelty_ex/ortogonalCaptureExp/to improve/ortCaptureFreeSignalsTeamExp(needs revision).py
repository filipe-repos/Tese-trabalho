...
def simula1(net, preds, prey, height, width, ticks):
    ...
    for count in range(ticks):
        ...
        outputsbruto, newsignals = ann_inputs_outputs_signal_t_T(tpreds, signals, tprey, net)
        signals = newsignals
        outputs = []
        n=0
...
def eval_fitness1(net, preds_def, theprey, height, width, ticks):
    ...
    for count in range(ticks): #(500 * 2 / 10) * (3/2) = 150
        ...
        outputsbruto, newsignals = ann_inputs_outputs_signal_T(preds, signals, prey, net)
        signals = newsignals
        outputs = []
        n=0
        #for n in range(0, (n_preds-1)*5 +1, 5):
